...
// [titulo, slug, subtitulo, conteudo, imagemDestaque, editoriaIndex (1-9), autorIndex (1-4)]
const noticias = [
...
const publicidades = [
    ['Banner Sidebar Top', 'sidebar_top', 'https://via.placeholder.com/300x250', '#'],
    ['Banner Feed 1', 'feed_posicao_1', 'https://via.placeholder.com/970x150', '#'],
    ['Banner Feed 2', 'feed_posicao_2', 'https://via.placeholder.com/970x150', '#']
];

async function main() {
    console.log('🌱 Iniciando seed do banco de dados (Prisma)...\n');

    await prisma.noticia.deleteMany();
    await prisma.publicidade.deleteMany();
    await prisma.editoria.deleteMany();
    await prisma.autor.deleteMany();

    console.log('📂 Criando editorias...');
    for (const [nome, slug] of editorias) {
        await prisma.editoria.create({ data: { nome, slug } });
        console.log(`  ✓ ${nome}`);
    }

    console.log('👤 Criando autores...');
    for (const [nome, email, avatar, bio] of autores) {
        await prisma.autor.create({ data: { nome, email, avatar, bio } });
        console.log(`  ✓ ${nome}`);
    }

    const editoriasList = await prisma.editoria.findMany({ orderBy: { id: 'asc' } });
    const autoresList = await prisma.autor.findMany({ orderBy: { id: 'asc' } });

    console.log('📰 Criando notícias...');
    for (const [titulo, slug, subtitulo, conteudo, imagemDestaque, edIdx, autIdx] of noticias) {
        await prisma.noticia.create({
            data: {
                ...
                editoriaId: editoriasList[edIdx - 1].id,
                autorId: autoresList[autIdx - 1].id
            }
        });
    }
    ...
    for (const [nome, posicao, imagem, link] of publicidades) {
        await prisma.publicidade.create({ data: { nome, posicao, imagem, link, ativo: true } });
        console.log(`  ✓ ${nome}`);
    }
