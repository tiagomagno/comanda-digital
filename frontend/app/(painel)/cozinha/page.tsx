...
// SLA da cozinha em segundos
const SLA_ATENCAO = 10 * 60;  // 10min → amarelo
const SLA_URGENTE = 15 * 60;  // 15min → vermelho
...
export default function CozinhaPage() {
    ...
    // Tick por segundo para o timer ao vivo
    useEffect(() => {
    ...
    const carregarPedidos = useCallback(async () => {
        try {
            ...
            const response = await fetch(`${config.apiUrl}/cozinha/pedidos?destino=COZINHA`, {
    ...
    const atualizarStatus = async (pedidoId: string, novoStatus: string) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${config.apiUrl}/cozinha/pedidos/${pedidoId}`, {
                method: 'PUT',
    ...
    // Tempo médio real dos pedidos em preparo
                    ...
                    {pedido.itens.map((item) => (
                        <div key={item.id} className="bg-gray-50 rounded-lg p-3">
                            <div className="flex items-center gap-3">
                                <span className="bg-blue-600 text-white px-3 py-1 rounded-full text-base font-bold min-w-[46px] text-center">
                                    {item.quantidade}x
                                </span>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 text-xs text-gray-500">
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-green-500 inline-block" /> Até 10min</span>
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-amber-400 inline-block" /> 10–15min</span>
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-red-500 inline-block" /> Acima de 15min</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-red-50/50">
                        <h2 className="font-semibold text-gray-900 flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-red-500" /> Aguardando
                        </h2>
                        <span className="bg-red-100 text-red-700 px-2.5 py-0.5 rounded-full text-sm font-medium">{pedidos.novos.length}</span>
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-amber-50/50">
                        <h2 className="font-semibold text-gray-900 flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-amber-500" /> Em Preparo
                        </h2>
                        <span className="bg-amber-100 text-amber-800 px-2.5 py-0.5 rounded-full text-sm font-medium">{pedidos.emPreparo.length}</span>
                    </div>
                    <div className="p-4 flex-1 overflow-y-auto space-y-4 min-h-[200px]">
